...
parse_variable_length_1_byte_test()->
    ...
    State = #parse_state { buffer = <<>>, readfun = ReadFun, max_buffer_size = 1000000 },
    ?assertEqual({ok, 1, <<>>}, mqttl_parser:parse_variable_length(State)).

parse_variable_length_1_byte_2_test()->
    Length = <<127:8>>,
    ReadFun = fun(_) -> {ok, Length } end,
    State = #parse_state { buffer = <<>>, readfun = ReadFun, max_buffer_size = 1000000 },
    ?assertEqual({ok, 127, <<>>}, mqttl_parser:parse_variable_length(State)).

parse_variable_length_2_bytes_test()->
    Length = <<193:8,2:8>>,
    ReadFun = fun(_) -> {ok, Length } end,
    State = #parse_state { buffer = <<>>, readfun = ReadFun, max_buffer_size = 1000000 },
    ?assertEqual({ok, 321, <<>>}, mqttl_parser:parse_variable_length(State)).
...
parse_variable_length_4_bytes_test()->
    ...
    ReadFun = fun(_) -> {ok, Length } end,
    State = #parse_state { buffer = <<>>, readfun = ReadFun, max_buffer_size = 1000000 },
    ?assertEqual({ok, 268435455, <<>>}, mqttl_parser:parse_variable_length(State)).

parse_variable_length_chunked_test()->
    ParseProcess = initialize_parse_process(<<>>, fun mqttl_parser:parse_variable_length/1),
...
parse_chunked_packet_test()->
    ...
    ParseProcess = initialize_parse_process(<<>>,fun mqttl_parser:parse_packet/1),
...
parse_2_consecutive_packets_test()->
    ...
    Binary = <<(mqttl_builder:build_packet(OriginalPacket1))/binary,
               (mqttl_builder:build_packet(OriginalPacket2))/binary>>,
    S = #parse_state{buffer = Binary, max_buffer_size = 100000, readfun = undefined},
    {ok,ParsedPacket1,S1} = mqttl_parser:parse_packet(S),
    {ok,ParsedPacket2,_S2} = mqttl_parser:parse_packet(S1),
...
test_binary_packet_for_error(Binary) ->
    S = #parse_state{buffer = Binary, max_buffer_size = 10000000, readfun = undefined},
    ?assertMatch({error, _}, mqttl_parser:parse_packet(S)).

test_packetfor_error(OriginalPacket,Reason) ->
    Binary = mqttl_builder:build_packet(OriginalPacket),
    S = #parse_state{buffer = Binary, max_buffer_size = 10000000, readfun = undefined},
    ?assertMatch({error, Reason}, mqttl_parser:parse_packet(S)).
