...
build_variable_length_0_test() ->
  Result = mqttl_builder:build_var_length(0),
  ?assertEqual(Result, <<0:8>>).

build_variable_length_127_test() ->
  Result = mqttl_builder:build_var_length(127),
  ?assertEqual(Result, <<127:8>>).

build_variable_length_128_byte_test() ->
  Result = mqttl_builder:build_var_length(128),
  ?assertEqual(Result, <<128:8,1:8>>).

build_variable_length_2_byte_test() ->
  Result = mqttl_builder:build_var_length(321),
  ?assertEqual(Result, <<193:8,2>>).

build_variable_length_max_test() ->
  Result = mqttl_builder:build_var_length(268435455),
  ?assertEqual(Result, <<16#FFFFFF7F:32>>).

build_variable_length_overflow_test() ->
  ?assertException(throw, variable_length_too_large, mqttl_builder:build_var_length(268435456)).
...
bild_CONNECT_test() ->
    ?assertEqual(<<1:4,0:4,
                   ...
                   <<"Password">>/binary
                 >>,
      mqttl_builder:build_packet(#'CONNECT'{client_id = <<"0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ">>,
                                                      clean_session = true,
                                                      keep_alive = 10,
                                                      username = <<"Username">>,
                                                      password = <<"Password">>,
...
build_CONNACK_test() ->
    ?assertEqual(<<2:4,0:4,
                   ...
                   1:8,0:8
                 >>,
      mqttl_builder:build_packet(#'CONNACK'{return_code = ?CONNACK_ACCEPTED,
...
build_CONNACK_Unacceptable_Protocol_test() ->
    ?assertEqual(<<2:4,0:4,
                   ...
                   0:8,1:8
                 >>,
      mqttl_builder:build_packet(#'CONNACK'{return_code = ?CONNACK_UNACCEPTABLE_PROTOCOL,
                                                      session_present = true})).
...
build_CONNACK_Identifier_Rejected_test() ->
    ?assertEqual(<<2:4,0:4,
                   ...
                   0:8,2:8
                 >>,
      mqttl_builder:build_packet(#'CONNACK'{return_code = ?CONNACK_IDENTIFIER_REJECTED,
                                                      session_present = true})).
...
build_CONNACK_Server_Unavailable_test() ->
    ?assertEqual(<<2:4,0:4,
                   ...
                   0:8,3:8
                 >>,
      mqttl_builder:build_packet(#'CONNACK'{return_code = ?CONNACK_SERVER_UNAVAILABLE,
                                                      session_present = true})).
...
build_CONNACK_BAD_USERNAME_PASSWORD_test() ->
    ?assertEqual(<<2:4,0:4,
                   ...
                   0:8,4:8
                 >>,
      mqttl_builder:build_packet(#'CONNACK'{return_code = ?CONNACK_BAD_USERNAME_OR_PASSWORD,
                                                      session_present = true})).
...
build_CONNACK_UNAUTHORIZED_test() ->
    ?assertEqual(<<2:4,0:4,
                   ...
                   0:8,5:8
                 >>,
      mqttl_builder:build_packet(#'CONNACK'{return_code = ?CONNACK_UNAUTHORIZED,
                                                      session_present = true})).
...
build_PUBLISH_test() ->
    ?assertEqual(<<3:4,1:1,2:2,1:1,
                   ...
                   1:16,2:16
                 >>,
      mqttl_builder:build_packet(#'PUBLISH'{packet_id = 10,
                                                       content = <<1:16,2:16>>,
                                                       dup = true,
                                                       qos = 2,
                                                       retain = true,
...
build_PUBACK_test() ->
    ?assertEqual(<<4:4,0:4,
                   ...
                   10:16
                 >>,
      mqttl_builder:build_packet(#'PUBACK'{packet_id = 10})).
...
build_PUBREC_test() ->
    ?assertEqual(<<5:4,0:4,
                   ...
                   10:16
                 >>,
      mqttl_builder:build_packet(#'PUBREC'{packet_id = 10})).
...
build_PUBREL_test() ->
    ?assertEqual(<<6:4,2:4,
                   ...
                   10:16
                 >>,
      mqttl_builder:build_packet(#'PUBREL'{packet_id = 10})).
...
build_PUBCOMP_test() ->
    ?assertEqual(<<7:4,0:4,
                   ...
                   10:16
                 >>,
      mqttl_builder:build_packet(#'PUBCOMP'{packet_id = 10})).
...
build_SUBSCRIBE_test() ->
    ?assertEqual(<<8:4,2:4,
                 ...
                 >>,
      mqttl_builder:build_packet(#'SUBSCRIBE'{
                     packet_id = 10,
                    subscriptions = [
                        {<<"a/b">>, 1},
                        {<<"c/d">>, 2}
...
build_SUBACK_test() ->
    ?assertEqual(<<9:4,0:4,
                   ...
                   0:8,2:8,128:8
                 >>,
      mqttl_builder:build_packet(#'SUBACK'{
                    packet_id = 10,
...
build_UNSUBSCRIBE_test() ->
    ?assertEqual(<<10:4,2:4,
                 ...
                 >>,
      mqttl_builder:build_packet(#'UNSUBSCRIBE'{
                     packet_id = 10,
...
build_UNSUBACK_test() ->
    ?assertEqual(<<11:4,0:4,
                   ...
                   10:16
                 >>,
      mqttl_builder:build_packet(#'UNSUBACK'{packet_id = 10})).

build_PINGREQ_test() ->
    ?assertEqual(<<12:4,0:4,
                   0:8
                 >>,
      mqttl_builder:build_packet(#'PINGREQ'{})).

build_PINGRESP_test() ->
    ?assertEqual(<<13:4,0:4,
                   0:8
                 >>,
      mqttl_builder:build_packet(#'PINGRESP'{})).

build_DISCONNECT_test() ->
    ?assertEqual(<<14:4,0:4,
                   0:8
                 >>,
      mqttl_builder:build_packet(#'DISCONNECT'{})).